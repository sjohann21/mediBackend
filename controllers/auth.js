...
exports.addJane = async (req, res) => {
  console.log("controllers ... addJane")
  try {
    const jane = await new Jane(req.body);
    console.log("req.body = ", req.body);
//user - changed to jane
...
exports.addClinic = async (req, res) => {
  console.log("controllers ... addClinic")
  try {
    const clinic = await new Clinic(req.body);
    console.log("req.body = ", req.body);

    await clinic.save((err, clinic) => {
      console.log("*** clinic = ", clinic)
      if (err) {
        console.log(" *** err = ", err)
        return res.status(400).json({
          error: "AUTH CLINIC: Email is taken"
        });
      }
